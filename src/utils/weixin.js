...
export function oAuth(callback) {
  ...
  // return
  if (memberId) {
    axiosApi(`${apiUrl}Weixin/wx_member?memberId=${memberId}`, 'getLogin').then(v => {
  
      if (v.all.status == false) {
        localStorage.clear()
      ...
      console.log(`我是openId${v.info[0].openid}`)
      localStorage.setItem('openId', v.info[0].openid);
      localStorage.setItem('memberId', v.info[0].Member.ID);
      localStorage.setItem('memberName', v.info[0].Member.MemberName);
      localStorage.setItem('MemberTel', v.info[0].Member.MemberTel);

      localStorage.setItem('memberType', v.info[0].Member.MemberType);
      localStorage.setItem('memberEmpid', v.info[0].Member.MemberEmpid);
      localStorage.setItem('memberGrpid', v.info[0].Member.MemberGrpid);

      localStorage.setItem('MemberZwno', v.info[0].Member.MemberZwno)
      localStorage.setItem('MemberGrpName', v.info[0].Member.MemberGrpName)
      localStorage.setItem('memberAvatar', v.info[0].Member.MemberPhotoHead);

      callback()
      
      // 执行完延时刷新页面解决在接口文件里取不到openID memberID的错误
      // setTimeout(()=>{
      //   location.reload()
      // }, 500)
...
export function wxInit(callback) {
  axiosApi(`${apiUrl}server/getJsApiSign`,'post').then(v => {
    wx.config({
